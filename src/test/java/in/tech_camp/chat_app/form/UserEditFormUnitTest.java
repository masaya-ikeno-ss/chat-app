...
public class UserEditFormUnitTest {
  private EditUserForm userEditForm;
  ...
  class ユーザー情報を編集できる場合 {
    ...
    public void 全てデータが存在すれば保存できること () {
      Set<ConstraintViolation<EditUserForm>> violations = validator.validate(userEditForm, ValidationPriority1.class);
  ...
  class ユーザー情報を編集できない場合 {
    ...
    public void emailは無効なメールでは登録できない() {
      ...
      Set<ConstraintViolation<EditUserForm>> violations = validator.validate(userEditForm, ValidationPriority2.class);
      assertEquals(1, violations.size());
